...
import type { SinhVienProfile, ApiResponse } from "../../apis/types/user";
...
export function useGetProfile(
  ...
  const [user, setUser] = useState<SinhVienProfile | null>(null);
  ...
  const loadUserProfile = useCallback(async () => {
    ...
    try {
      ...
      const response: ApiResponse<SinhVienProfile> = await getProfile(token);
      if (response.data) {
